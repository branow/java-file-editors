...
public class SerializationTest {
    ...
    @Test
    public void testSerializationCharacter() throws SerializationException, IOException {
        Character[] expected = {'d', '5', '0'};
        Serialization<Character[]> xml = Serialization.serializationXML(dir, name);
        xml.serializate(expected);
        Character[] actual = xml.deserializate();
        for (int i=0; i<expected.length; i++) {
            Assertions.assertEquals(expected[i], actual[i]);
        }
    ...
    @Test
    public void testSerializationString() throws SerializationException, IOException {
        String[] expected = {"dsaf d q  \n", "       ", ""};
        Serialization<String[]> xml = Serialization.serializationXML(dir, name);
        xml.serializate(expected);
        String[] actual = xml.deserializate();
        for (int i=0; i<expected.length; i++) {
            Assertions.assertEquals(expected[i], actual[i]);
        }
        Assertions.assertArrayEquals(expected, actual);
    ...
    @Test
    public void testSerializationIntegerMatrix() throws SerializationException, IOException {
    ...
    @Test
    public void testSerializationObjectMatrix() throws SerializationException, IOException {
        Card[][] expected = getCardsMat(0);
        Serialization<Card[][]> xml = Serialization.serializationXML(dir, name);
        xml.serializate(expected);
        Card[][] actual = xml.deserializate();
        Assertions.assertTrue(Arrays.deepEquals(expected, actual));
        Assertions.assertEquals(XML_TEXT_MATRIX_CARDS_0, new FileText(dir + File.separator + name + ".xml").read());
    }

    @Test
    public void testSerializationCard() throws SerializationException, IOException {
        Card expected = getCard(0);
        Serialization<Card> xml = Serialization.serializationXML(dir, name);
        xml.serializate(expected);
        Card actual = xml.deserializate();
        Assertions.assertEquals(expected, actual);
        Assertions.assertEquals(XML_TEXT_CARD_0, new FileText(dir + File.separator + name + ".xml").read());
    }
